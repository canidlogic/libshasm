...
The remainder of this section describes the string decoding algorithm.  See subsection 4.1 for the specific worklist of this roadmap stage.

In order to implement the greedy matching required for using the decoding map, this roadmap stage will require a more sophisticated buffer than the single-character pushback buffer offered by shasm_input.  However, to limit the complexities of dealing with two levels of nonlinear buffers, the more sophisticated buffer reader will be confined to one subfunction of the decoder, and the rest of the decoder will simply use the pushback buffer.  The subfunction that uses the sophisticated buffer is the "inner" decoder, while the rest of the decoder that only requires the pushback buffer is the "outer" decoder.

The interface of the inner decoder takes the same parameters as the outer decoder (the block reader, the input filter statck, and the string format structure).  In addition, the inner decoder takes a temporary buffer object and the sophisticated buffer object.  Neither of these objects are used to pass parameters, but they are allocated and initialized in the outer decoder so that they can be used across multiple calls to the inner decoder for sake of efficiency.

The inner decoder will decode zero or more entity codes from the input stream, passing these through to the encoder function.  The inner decoder will also handle numeric escapes.  The inner decoder returns when it encounters something it can't decode.  The return value is successful if the inner decoder managed to empty the sophisticated buffer such that the outer decoder can make another attempt at decoding what the inner decoder stopped at.  The return value is an error in any other case -- if the sophisticated buffer could not be emptied, it means that there is data that the outer decoder wouldn't be able to interpret, either.  (This is guaranteed by the kinds of decoding keys that the inner decoding loop will ignore, in line with section 5.1 of draft 3V:C4-5 of the Shastina Specification.)

The outer decoder has a loop that begins by calling the inner decoder.  If the inner decoder returns successfully, the outer decoder will then attempt to interpret the data (direct from the input filter stack) using the built-in keys.  The built-in keys include the terminal characters (which cause the decoding loop to end successfully), and, if an input override is selected, sequences of bytes with their most significant bit set -- which are decoded according to the override and passed through to the encoding function.  If the loop hasn't terminated on error or on account of a terminal built-in key, it then loops back and calls the inner decoder again.
